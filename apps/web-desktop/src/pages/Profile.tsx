...
export function Profile() {
  ...
  return (
    <div className="inner-page profile-page">
      <header className="profile-hero"><div className="profile-avatar">{user?.nickname?.slice(0, 1) ?? '吾'}</div><div><p className="eyebrow">我的山居 · 不设手机号</p><h1>{user?.nickname ?? '山中访客'}</h1><small>访客身份会保存在当前浏览器 · 记录由山问服务端保管</small></div><Link to="/lamp" className="profile-lamp"><i />{user?.isLamp ? '长明灯已亮' : '供一盏长明灯'}<span>→</span></Link></header>
      <div className="content-wrap profile-layout">
        <aside className="profile-nav"><p className="aside-label">我的山居</p><a className="active">命盘总览</a><Link to="/profile/qians">我的签文</Link><Link to="/wishes">我的愿望</Link><Link to="/chat">问室记录</Link><Link to="/palm">手相观照</Link></aside>
        <section>
          ...
          <div className="profile-cards"><Link to="/profile/qians"><span>签</span><div><small>我的签文</small><b>回看曾经照见的念头</b></div><i>→</i></Link><Link to="/chat"><span>问</span><div><small>问卦记录</small><b>继续未说完的对谈</b></div><i>→</i></Link><Link to="/wishes"><span>愿</span><div><small>我的愿望</small><b>看看正在走近的事情</b></div><i>→</i></Link></div>
        </section>
      </div>
    </div>
  );
