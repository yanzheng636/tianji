...
export interface WikiEvidencePage { items: WikiEvidence[]; total: number; offset: number; limit: number }
...
export type ChatStreamEvent = { type: 'session'; sessionId: string; title: string } | { type: 'delta'; text: string } | { type: 'citation'; citation: Citation } | { type: 'done'; messageId: string; sessionId: string } | { type: 'error'; message: string };
